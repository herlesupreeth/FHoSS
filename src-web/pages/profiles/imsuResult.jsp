	...
	<tr id="bound_bottom_black">
		<td valign="top" bgcolor="#FFFFFF"><logic:notEmpty name="result">
			<table class="as" width="500">
				<tr class="header">
					<td class="header"><bean:message key="imsu.head.name" /></td>
					<td class="header"><bean:message key="form.head.action" /></td>
				</tr>
				<logic:iterate name="result" id="imsu"
					type="de.fhg.fokus.hss.model.Imsu" indexId="ix">

					<tr class="<%= ix.intValue()%2 == 0 ? "even" : "odd" %>">
						<td><bean:write name="imsu" property="name" /></td>
						<td align="center">
							<table>
								<tr>
									<td>
									<form method="post" action="/hss.web.console/imsuShow.do"
										target="content" style="text-align: center"><input type="hidden"
										name="imsuId"
										value="<bean:write name="imsu" property="imsuId" />"> <input
										type="image" src="/hss.web.console/images/edit_small.gif"></form>
									</td>
									<% if(request.isUserInRole(SecurityPermissions.SP_IMSU)) { %>
										<td>
										<form method="post" action="/hss.web.console/imsuDelete.do"
											target="content" style="text-align: center"><input type="hidden"
											name="imsuId"
											value="<bean:write name="imsu" property="imsuId" />"> <input
				...
				<tr>
					<td colspan="3" class="header"><script type="text/javascript"
						...
						function submitForm(pageId){
							document.imsuSearchForm.page.value = pageId;
							document.imsuSearchForm.submit();
						}
					</script> <html:form action="/imsuSearch">
						<table>
							<tr>
				...
				%></td>
								...
								<html:hidden property="name"></html:hidden> <html:hidden
									...
									onchange="javascript:document.imsuSearchForm.submit();">
									<option value="20"
