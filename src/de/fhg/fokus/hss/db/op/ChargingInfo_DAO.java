...
public class ChargingInfo_DAO {
	...
	public static void update(Session session, ChargingInfo charging_info){
		session.save(charging_info);
	}
	
	public static ChargingInfo get_by_ID(Session session, int id){
		Query query;
		query = session.createSQLQuery("select * from charging_info where id=?")
	...
	public static ChargingInfo get_by_Name(Session session, String name){
	...
	public static Object[] get_by_Wildcarded_Name(Session session, String name, 
		...
		Query query;
		query = session.createSQLQuery("select * from charging_info where name like ?")
			.addEntity(ChargingInfo.class);
		query.setString(0, "%" + name + "%");
	...
	public static List get_all(Session session){
		Query query = session.createSQLQuery("select * from charging_info")
			.addEntity(ChargingInfo.class);
		return query.list();
	}
	...
	public static Object[] get_all(Session session, int firstResult, int maxResults){
		Query query;
		query = session.createSQLQuery("select * from charging_info")
			.addEntity(ChargingInfo.class);
	...
	public static int delete_by_ID(Session session, int id){
		Query query = session.createSQLQuery("delete from charging_info where id=?");
		query.setInteger(0, id);
