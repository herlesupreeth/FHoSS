...
public class TP_DAO {
	...
	public static TP get_by_ID(Session session, int id){
		Query query;
		query = session.createSQLQuery("select * from tp where id=?")
			.addEntity(TP.class);
	...
	public static TP get_by_Name(Session session, String name){
		Query query;
	...
	public static int delete_by_ID(Session session, int id){
		Query query = session.createSQLQuery("delete from tp where id=?");
		query.setInteger(0, id);
	...
	public static Object[] get_all(Session session, int firstResult, int maxResults){
		Query query;
		query = session.createSQLQuery("select * from tp")
			.addEntity(TP.class);
